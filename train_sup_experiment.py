...
def main():
    ...
    while not plateau_scheduler.is_done():
        ...
        train(
            model,
            optimizer,
            plateau_scheduler,     # from Mike's code
            #lr_scheduler,
            criterion,
            train_loader_sup,
            epoch,
...
def train(
    ...
    for step, tensor_dict in enumerate(data_loader):    # range(len(data_loader)):
        ...
        image, label = tensor_dict #data_loader_iter.next()         #load image and corresponding label from dataloader iterator    NOTE: Add third argument: img_path to debug image saving
