...
def main():
    ...
    modules_back = [model.encoder]                      #Is this making the backbone? -VP
    if cfg["net"].get("aux_loss", False):
        modules_head = [model.auxor, model.decoder]     #What is this doing? -VP
    else:
...
def train(
    ...
    epoch,
    tb_logger,
): # the beginning of the train method
    ...
    for step, tensor_dict in enumerate(data_loader): #range(len(data_loader)):
        ...
        image, label = tensor_dict #data_loader_iter.next()         #load image and corresponding label from dataloader iterator
        batch_size, h, w = label.size()
        image, label = image.cuda(), label.cuda()
        outs = model(image)                 # logit output that is height x width x class number (each pixel has one hot encoding which creates third dimension)
        pred = outs["pred"]                 # check if takes argmax of each one hot vector of every pixel
