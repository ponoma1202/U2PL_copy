...
def train(
    ...
    for step, tensor_dict in enumerate(data_loader):
        ...
        #         skimage.io.imsave(os.path.join(args.prediction_dir, image_name), np.uint8(image_),
        ...
        if i_iter % 100 == 0 and rank == 0:
            ...
            logger.info(        # TODO: Change back to logging after debugging
                "Iter [{}/{}]\t"
...
def validate(
    ...
    # start Mike's code
    logger.info('Evaluating model against validation data')        # TODO: change back to logging
    start_time = time.time()
    # end Mike's code
    ...
    for i, iou in enumerate(iou_class):
        logger.info(" * class [{}] IoU {:.2f}".format(i, iou * 100))       # TODO: Change bacj to logging
    logger.info(" * epoch {} mIoU {:.2f}".format(epoch, mIoU * 100))       # TODO: change back to logging
