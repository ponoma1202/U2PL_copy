...
def train(
    ...
    for step, tensor_l_dict in enumerate(loader_l): #range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 1):
            ...
            student_per_class_accuracy = class_accuracy(label_u_aug, label_u, batch_size)
            train_stats.add(epoch, 'student_per_class_accuracy', student_per_class_accuracy)
            ...
            student_per_class_accuracy = class_accuracy(pred_student, label_u, batch_size)
            train_stats.add(epoch, 'student_per_class_accuracy', student_per_class_accuracy)
...
def class_accuracy(pred, label, batch_size):
    batch_class_accuracy = []
    num_classes = cfg["net"]["num_classes"]

    for img in range(batch_size):
        pred_cpy = pred[img].cpu().detach()
        label_cpy = label[img].cpu().detach()
        img_accuracy = torch.zeros(num_classes)
        ...
        for id in range(0, num_classes):
            ...
            try:
                total_accuracy = np.count_nonzero(per_pixel_accuracy) / np.count_nonzero(
                    np.logical_or(pred_mask, label_mask))  # maybe edit denominator to just count_nonzero of label
            except:
                total_accuracy = 0
            img_accuracy[id] = total_accuracy
        batch_class_accuracy.append(img_accuracy)

    return torch.mean(torch.Tensor(batch_class_accuracy), dim=0)
